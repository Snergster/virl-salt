...
ank preview port:
  cmd.run:
    - name: 'crudini --set /etc/virl/common.cfg host ank_preview_port {{ virl.topology_editor_port }}'
    - require:
      - pip: VIRL_CORE
...
{% if virl.web_editor %}
  cmd.run:
    - name: 'crudini --set /etc/virl/common.cfg host topology_editor_port {{ virl.topology_editor_port }}'
{% else %}
  file.replace:
