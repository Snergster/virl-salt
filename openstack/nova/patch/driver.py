...
CONF.import_opt('serial_port_proxyclient_address', 'nova.console.serial')
...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def __init__(self, virtapi, read_only=False):
        ...
        serial_port_range = CONF.libvirt.serial_port_range
        if serial_port_range is None:
            serial_port_range = '10000:20000'

        serial_port_range = serial_port_range.split(':', 1)
        try:
            serial_port_range = [int(serial_port_range[0]),
                               int(serial_port_range[1])]
        except ValueError:
            LOG.warn(_("serial_port_range should be <num>:<num> but "
                       "given value %(serial_port_range) could "
                       "not be parsed") %
                       {'serial_port_range': serial_port_range})
            serial_port_range = [10000, 20000]  # the default
        self.serial_tcp_ports = \
            port.PortAllocator(serial_port_range,
                               CONF.serial_port_proxyclient_address)
    ...
    def get_serial_port(self, instance, serial_port):
        if not CONF.serial_port_enabled:
            return None
        ...
        # In the devices block, the serial stanzas look like this
        # for any serial port that's available over TCP so that we
        # can serve it:
        ...
        # If we look through the serial ports that are on TCP...
        for serial in tree.findall("./devices/serial[@type='tcp']"):
            # .. can we find the one we would like?
            source = serial.find("./source[@mode='bind']")
            if (source is not None
                and serial.find("./target[@port='" +
                                str(serial_port) + "']") is not None):
                tcp_port = source.get("service")
                break

        if tcp_port is None:
            # If a port has no XML definition, or no definition that's
            # listening on TCP - we can't share it with an end user
            return None
    ...
    def get_guest_config(self, instance, network_info, image_meta,
        ...
        def get_serial_ports(image_meta):

            # TODO(ijw): packaged away so that we can extend the
            # 'serial' format to e.g. mix a console and serial ports
            if image_meta is None:
                LOG.debug('no image_meta')
                yield 'console'
                return
            props = image_meta.get('properties', {})
            if 'serial' not in props:
                LOG.debug('serial property not set')
                yield 'console'
                return
            else:
                try:
                    s = props['serial']
                    LOG.debug('number of serial ports listed as %s' % s)
                    x = int(s)
                except ValueError:
                    LOG.error('serial ports value "%s" makes no sense' % s)
                    yield 'console'
                    return
        ...
        if CONF.libvirt.virt_type in ("qemu", "kvm"):
            for f in get_serial_ports(image_meta):
                if f == 'console':  # there will only be one of these
                    ...
                    consolelog.source_path = \
                        self._get_console_log_path(instance)
                    ...
                    serial_port = vconfig.LibvirtConfigGuestSerial()
                    serial_port.target_port = f
                    serial_port.protocol_type = "telnet"
                    serial_port.type = "tcp-listener"
                    (serial_port.listen_host,
                     serial_port.listen_port) = \
                        self.serial_tcp_ports.get_port()
                    guest.add_device(serial_port)
        else:
