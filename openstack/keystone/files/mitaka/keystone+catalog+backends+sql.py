    ...
    def get_catalog(self, user_id, tenant_id):
        ...
        with sql.session_for_read() as session:
            ...
            for endpoint in endpoints:
                ...
                # if there are multiple versions of same service, a random one
                # ends up in the result (catalog). This happens with identity
                # as that has the endpoints actually listed
                if service_type == 'identity' and not url.endswith('2.0'):
                    continue
                default_service = {
                    'id': endpoint['id'],
