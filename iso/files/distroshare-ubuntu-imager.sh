...
# Modified to support VIRL ISO Installations

#GPL2 License

VERSION="1.0.15"

echo ###### Distroshare Ubuntu Imager $VERSION   ######
...
#apt-get -y remove ubiquity-slideshow-ubuntu
...
# Replace the slideshow with one customized for VIRL

tar -xzvf ~/ub.new.tar.gz -C /home/distroshare/rootfs/usr/share/ubiquity-slideshow/
...
menuentry \"VIRL Live Installer\" {
linux /casper/vmlinuz boot=casper $KERNEL_PARAMS quiet splash --
initrd /casper/initrd.img
}

menuentry \"VIRL Safe-Mode Installer\" {
...
grub-mkrescue -o "${WORK}"/virl-live-cd.iso "${CD}"
mv "${WORK}"/virl-live-cd.iso ~/
echo "The ISO is at home/virl//virl-live-cd.iso"
